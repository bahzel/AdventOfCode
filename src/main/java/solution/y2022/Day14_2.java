...
public class Day14_2 extends MapSolution<Set<Point>> {
	...
	protected String computeSolution(Set<Point> points) {
		...
		var maximumY = points.stream().mapToInt(Point::getY).max().orElseThrow() + 1;
		...
		while (true) {
			...
			while (true) {
				if (currentPoint.getY() == maximumY) {
					break;
				} else if (!points.contains(currentPoint.getLowerNeighbour())) {
