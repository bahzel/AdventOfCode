...
public class Day17_1 extends MapSolution<List<Direction>> {
	...
	protected String computeSolution(List<Direction> directions) {
		...
		for (var i = 0; i < 2022; i++) {
			var currentTile = tiles.get(i % tiles.size());
...
class Tile {
	...
	public Tile(List<Point> squares) {
		this.squares = squares;
		height = squares.stream().mapToInt(Point::getY).max().orElseThrow() + 1;
	}
}