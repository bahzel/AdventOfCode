...
public class Day24_2 extends Solution {
	...
	protected String doSolve() {
		...
		minY = 1;
		maxY = input.size() - 2;
		return findPath(plateau, new Point(1, 0), new Point(maxX, maxY + 1))
				+ findPath(plateau, new Point(maxX, maxY + 1), new Point(1, 0))
				+ findPath(plateau, new Point(1, 0), new Point(maxX, maxY + 1)) + "";
	}

	private int findPath(List<Pair<Point, Direction>> plateau, Point start, Point goal) {
		var currentPositions = Set.of(start);

		for (var i = 0;; i++) {
			var newPlateau = computeNextRound(plateau);
			plateau.clear();
			plateau.addAll(newPlateau);
			...
			for (var currentPosition : currentPositions) {
				if (currentPosition.getY() < minY) {
					nextPositions.add(currentPosition.getLowerNeighbour());
					nextPositions.add(currentPosition);
					continue;
				} else if (currentPosition.getY() > maxY) {
					nextPositions.add(currentPosition.getUpperNeighbour());
					nextPositions.add(currentPosition);
					continue;
				}
				...
				} else if (currentPosition.getUpperNeighbour().equals(goal)) {
					return i + 1;
				}
