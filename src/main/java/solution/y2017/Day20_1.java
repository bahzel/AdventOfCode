...
public class Day20_1 extends MapSolution<List<Pair<Point3D, Point3D>>> {
	...
	protected void transformInstruction(String instruction, List<Pair<Point3D, Point3D>> point3DS) {
		...
		point3DS.add(Pair.of(new Point3D(Integer.parseInt(acceleration[0]), Integer.parseInt(acceleration[1]),
	...
	protected String computeSolution(List<Pair<Point3D, Point3D>> point3DS) {
		...
		var slowestPoint = point3DS.stream()
								   .filter(point -> point.getLeft().computeManhattanDistance(zero)
										   == slowestAcceleration)
								   .min((p1, p2) ->
										   p1.getLeft().getX() + 1000 * p1.getRight().getX() + p1.getLeft().getY()
												   + 1000 * p1.getRight().getY() + p1.getLeft().getZ()
