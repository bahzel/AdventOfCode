...
public class Day3_2 extends Solution {
	...
	private long getValueOver(long goal) {
		...
		for (int i = 1; true; i = i + 2) {
			for (int j = 0; j < i; j++) {
				var nextCoordinates = new Point(currentPoint.getCoordinates().getX() + 1,
						currentPoint.getCoordinates().getY());
			...
			for (int j = 0; j < i; j++) {
				var nextCoordinates = new Point(currentPoint.getCoordinates().getX(),
						currentPoint.getCoordinates().getY() - 1);
				var nextValue = computeValue(nextCoordinates, neighbours);
				neighbours.put(nextCoordinates, nextValue);
				currentPoint = new GridElement<>(nextValue, nextCoordinates);
				if (nextValue >= goal) {
