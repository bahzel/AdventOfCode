...
public class Day13_2 extends MapSolution<Map<Pair<String, String>, Integer>> {
	...
	protected void transformInstruction(String instruction, Map<Pair<String, String>, Integer> pairIntegerMap) {
		...
		pairIntegerMap.keySet().stream().map(Pair::getLeft).distinct().toList().forEach(person -> {
			pairIntegerMap.put(Pair.of("Me", person), 0);
			pairIntegerMap.put(Pair.of(person, "Me"), 0);
		});
	}
