...
public class Day19_2 extends MapSolution<Map<String, String>> {
	...
	protected void transformInstruction(String instruction, Map<String, String> stringStringMap) {
		...
		if (instruction.startsWith("8:")) {
			fixedInstruction = "8: 42 | 42 8";
		} else if (instruction.startsWith("11:")) {
			fixedInstruction = "11: 42 31 | 42 11 31";
		} else {
			fixedInstruction = instruction;
		}

		var instructions = fixedInstruction.split(": ");
			...
			if (keys.length == 3) {
				for (var i = 1; i < value.length() - 1; i++) {
					for (var j = i + 1; j < value.length(); j++) {
						if (doesMatch(instructions, keys[0], value.substring(0, i))
								&& doesMatch(instructions, keys[1], value.substring(i, j))
								&& doesMatch(instructions, keys[2], value.substring(j))) {
							CACHE.put(key + value, true);
							return true;
						}
					}
