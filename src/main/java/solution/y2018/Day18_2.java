...
public class Day18_2 extends GridSolution<LandScape> {
	...
	private long computeResourceValue() {
		return stream().filter(neighbour -> neighbour.getValue() == LandScape.TREE).count() * stream().filter(
				neighbour -> neighbour.getValue() == LandScape.LUMBERYARD).count();
	}
	...
	private void computeNextMinute() {
		stream().forEach(acre -> {
			...
			case LandScape.OPEN_GROUND:
				acre.setNextValue(countNeighboursWithLandscape(acre, LandScape.TREE) >= 3
						? LandScape.TREE
						: LandScape.OPEN_GROUND);
				break;
			case LandScape.TREE:
				acre.setNextValue(countNeighboursWithLandscape(acre, LandScape.LUMBERYARD) >= 3
						? LandScape.LUMBERYARD
						: LandScape.TREE);
				break;
			case LandScape.LUMBERYARD:
				acre.setNextValue(countNeighboursWithLandscape(acre, LandScape.LUMBERYARD) >= 1
						&& countNeighboursWithLandscape(acre, LandScape.TREE) >= 1
						? LandScape.LUMBERYARD
						: LandScape.OPEN_GROUND);
				break;
			}
