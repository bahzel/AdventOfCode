...
public class Day22_2 extends MapSolution<List<Triple<Point, Integer, Integer>>> {
	...
	protected void transformInstruction(String instruction, List<Triple<Point, Integer, Integer>> triples) {
		...
		triples.add(Triple.of(new Point(Integer.parseInt(coordinates[1]), Integer.parseInt(coordinates[2])),
	...
	protected String computeSolution(List<Triple<Point, Integer, Integer>> triples) {
		...
		var rowOfWall = triples	.stream()
								.filter(tile -> tile.getMiddle() > 100)
								.findAny()
								.orElseThrow()
								.getLeft()
								.getY();
		var columnOfGap = triples	.stream()
									.filter(tile -> tile.getMiddle() < 100 && tile.getLeft().getY() == rowOfWall)
									.map(tile -> tile.getLeft().getX())
									.mapToInt(x -> x)
									.max()
									.orElseThrow();
