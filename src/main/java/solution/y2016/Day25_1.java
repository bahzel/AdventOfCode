...
public class Day25_1 extends MapSolution<List<TriConsumer<Map<String, Integer>, AtomicInteger, StringBuilder>>> {
    ...
    @Override
    protected void transformInstruction(String instruction, List<TriConsumer<Map<String, Integer>, AtomicInteger, StringBuilder>> consumers) {
        var instructions = instruction.split(" ");
        switch (instructions[0]) {
            case "cpy":
                if (StringUtils.isNumeric(instructions[1])) {
                    consumers.add((registers, index, output) -> registers.put(instructions[2], Integer.parseInt(instructions[1])));
                } else {
                    consumers.add((registers, index, output) -> registers.put(instructions[2], registers.get(instructions[1])));
                }
                break;
            case "inc":
                consumers.add((registers, index, output) -> registers.put(instructions[1], registers.get(instructions[1]) + 1));
                break;
            case "dec":
                consumers.add((registers, index, output) -> registers.put(instructions[1], registers.get(instructions[1]) - 1));
                break;
    ...
    private String execute(int initialValue, List<TriConsumer<Map<String, Integer>, AtomicInteger, StringBuilder>> consumers) {
