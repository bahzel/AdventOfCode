...
public class Day21_1 extends InstructionSolution<Consumer<StringBuilder>, StringBuilder> {
    ...
    protected StringBuilder initializeValue() {
        return new StringBuilder("abcdefgh");
    }
    ...
    protected Consumer<StringBuilder> transformInstruction(String instruction) {
        ...
        if ("swap".equals(instructions[0]) && "position".equals(instructions[1])) {
            ...
            return builder -> {
                var position = Integer.parseInt(instructions[2]);
                var move = builder.charAt(position);
                builder.deleteCharAt(position).insert(Integer.parseInt(instructions[5]), move);
            };
            ...
            return builder -> {
                ...
                rotateRight(index + 1 + (index > 3 ? 1 : 0), builder);
            };
        } else {
