...
public class Day1_1 extends InstructionSolution<Pair<Direction, Integer>, Pair<AtomicReference<Direction>, Point>> {
	...
	protected boolean performInstruction(Pair<Direction, Integer> directionIntegerPair,
		...
		case RIGHT ->
			atomicReferencePointPair.getRight()
									.setX(atomicReferencePointPair.getRight().getX() + directionIntegerPair.getRight());
		case LEFT ->
			atomicReferencePointPair.getRight()
									.setX(atomicReferencePointPair.getRight().getX() - directionIntegerPair.getRight());
		case UP ->
			atomicReferencePointPair.getRight()
									.setY(atomicReferencePointPair.getRight().getY() + directionIntegerPair.getRight());
		case DOWN ->
			atomicReferencePointPair.getRight()
									.setY(atomicReferencePointPair.getRight().getY() - directionIntegerPair.getRight());
		}
