...
public class Day24_2 extends GridSolution<Boolean> {
	...
	private long computeShortestDistance(int currentLocation, long currentCost, List<Integer> leftLocations,
			Map<Integer, Long> distanceMap) {
		if (leftLocations.size() == 1) {
			return distanceMap.get(currentLocation * 10 + leftLocations.getFirst())
					+ distanceMap.get(leftLocations.getFirst() * 10) + currentCost;
		}
		...
		for (var leftLocation : leftLocations) {
			...
			minimumDistance = Math.min(minimumDistance, computeShortestDistance(leftLocation,
					distanceMap.get(currentLocation * 10 + leftLocation) + currentCost, newLeftLocations, distanceMap));
		}
