...
public class Day18_2 extends MapSolution<List<boolean[]>> {
    private final int ROW_COUNT = 400000;
    ...
    protected String computeSolution(List<boolean[]> booleans) {
        ...
        return booleans.stream().flatMap(arr -> IntStream.range(0, arr.length)
                .mapToObj(i -> arr[i])).filter(value -> value).count() - 2 * ROW_COUNT + "";
    }
