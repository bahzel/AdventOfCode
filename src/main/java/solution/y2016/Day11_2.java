...
public class Day11_2 extends Solution {
	...
	protected String doSolve() {
		var currentRound = List.of("000101000002222");
		...
		while (true) {
			...
			var maximumScore = nextRound.stream()
										.map(this::computeScore)
										.mapToLong(Long::longValue)
										.max()
										.orElseThrow();
			currentRound = nextRound.stream()
									.filter(nextStep -> computeScore(nextStep) >= maximumScore * 95L / 100L)
									.toList();
		}
	}
	...
	private boolean isGoal(String facility) {
		return "333333333333333".equals(facility);
	}
