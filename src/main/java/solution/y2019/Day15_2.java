...
public class Day15_2 extends MapSolution<IntCodeInterpreter> {
	...
	protected String computeSolution(IntCodeInterpreter intCodeInterpreter) {
		var currentLocation = new GridElement<>(true, 0, 0);
		...
		thread.interrupt();
		return computeLongestDistance(oxygenLocation) + "";
	}
	...
	private int computeLongestDistance(GridElement<Boolean> startingLocation) {
		var maximumDistance = 0;
		var cache = new HashSet<GridElement<Boolean>>();
		Queue<Pair<GridElement<Boolean>, Integer>> queue = new LinkedList<>();
		queue.add(Pair.of(startingLocation, 0));
		while (!queue.isEmpty()) {
