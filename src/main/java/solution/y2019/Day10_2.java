...
public class Day10_2 extends GridSolution<Integer> {
	private final int ASTEROID_INDEX = 200;
	...
	protected String computeSolution() {
		stream().forEach(tile -> {
			var seenAsteroids = computeSeenAsteroids(tile);
			if (!seenAsteroids.isEmpty()) {
				tile.setValue(computeSeenAsteroids(tile).size());
			}
		});
		var laserLocation = stream().max(Comparator.comparingInt(GridElement::getValue)).orElseThrow();

		var destroyedAsteroids = 0;
		while (true) {
			var nextAsteroids = computeSeenAsteroids(laserLocation);
			if (destroyedAsteroids + nextAsteroids.size() < ASTEROID_INDEX) {
				destroyedAsteroids += nextAsteroids.size();
				nextAsteroids.forEach(tile -> tile.setValue(-1));
			} else {
				var destroyedAsteroidsList = new ArrayList<>(nextAsteroids);
				destroyedAsteroidsList.sort(Comparator.comparingDouble(
						asteroid -> laserLocation.getCoordinates().getDegreeTo(asteroid.getCoordinates())));
				var twoHundredth = destroyedAsteroidsList.get(ASTEROID_INDEX - destroyedAsteroids - 1);
				return twoHundredth.getCoordinates().getX() * 100 + twoHundredth.getCoordinates().getY() + "";
			}
		}
	}

	private Set<GridElement<Integer>> computeSeenAsteroids(GridElement<Integer> tile) {
		if (tile.getValue() == -1) {
			return new HashSet<>();
		}
